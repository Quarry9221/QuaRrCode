...
class Settings(BaseSettings):
    ...
    api_url: str = Field(
        default="http://localhost:8000", description="Base URL for FastAPI backend"
    )
    ...
    redis_url: Optional[str] = Field(
        default=None, description="Redis URL for caching or rate limiting"
    )
    max_qr_text_length: int = Field(
        default=2048, ge=1, le=4096, description="Maximum length of text for QR code"
    )
    ...
    max_file_size_mb: int = Field(
        default=10, ge=1, le=50, description="Maximum file size in MB for uploads"
    )
    log_level: str = Field(
        default="DEBUG",
    ...
    default_qr_settings: dict = Field(
        default={"fmt": "PNG", "size": 10, "fg": "black", "bg": "white"},
        description="Default settings for QR code generation",
    )
    allowed_qr_formats: List[str] = Field(
        default=["PNG", "SVG"], description="Supported QR code formats"
    )
    allowed_qr_sizes: List[int] = Field(
        default=[5, 10, 15, 20, 25], description="Supported QR code sizes"
    )
