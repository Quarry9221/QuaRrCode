...
class UserRepository:
        ...
        if db is None:
            async with AsyncSessionLocal() as session:
                return await UserRepository.get_or_create_user(
                    telegram_id, username, first_name, session
                )
    ...
    async def get_user_stats(
        telegram_id: int, db: AsyncSession = None
    ) -> Optional[dict]:
        if db is None:
            async with AsyncSessionLocal() as session:
...
class QRRepository:
    ...
    async def save_qr_code(
        telegram_id: int, text_content: str, settings: dict, db: AsyncSession = None
    ):
        if db is None:
            async with AsyncSessionLocal() as session:
                qr_code = QRCode(
