...
async def main():
    ...
    async with aiohttp.ClientSession() as http_session, async_session() as db_session:
        ...
        logger.debug(
            f"db_session in bot_data: {application.bot_data.get('db_session') is not None}"
        )

        application.add_handler(CommandHandler("start", StartHandler.start_command))
        application.add_handler(
            CallbackQueryHandler(CallbackHandler.handle_callback_query)
        )
        application.add_handler(
            MessageHandler(filters.TEXT & ~filters.COMMAND, MessageRouter.route_message)
        )
