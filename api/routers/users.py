...
async def create_user(user: UserCreate, db: AsyncSession = Depends(get_db), api_key: str = Depends(get_api_key)):
...
async def get_user_stats(telegram_id: int, db: AsyncSession = Depends(get_db), api_key: str = Depends(get_api_key)):
