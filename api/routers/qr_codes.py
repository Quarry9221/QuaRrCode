...
async def create_qr_code(
    ...
    if qr.format.upper() not in settings.allowed_qr_formats:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid format. Allowed: {settings.allowed_qr_formats}",
        )
    if qr.size not in settings.allowed_qr_sizes:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid size. Allowed: {settings.allowed_qr_sizes}",
        )
    if (
        qr.fg_color not in settings.allowed_colors
        or qr.bg_color not in settings.allowed_colors
    ):
        raise HTTPException(
            status_code=400, detail=f"Invalid color. Allowed: {settings.allowed_colors}"
        )
    ...
    db_qr = await QRRepository.save_qr_code(
        user_id=user.id, text_content=qr.text_content, settings=qr_settings, db=db
    )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=qr_code_{db_qr.id}.{qr.format.lower()}"
        },
    )
